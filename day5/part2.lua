...
function Entry:new(destination_start, source_start, range)
    ...
    entry.destination_start = destination_start and tonumber(destination_start) or nil
...
local seeds = {}
for seed_start, range in lines():gmatch("(%d+) (%d+)") do
    local entry = Entry:new(_, seed_start, range)
    table.insert(seeds, entry)
    print(seed_start, range)
end
...
while line do
    ...
    table.insert(maps, Map:new(from, to, entries))
end
